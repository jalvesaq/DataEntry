...
VarListDlg <- function(...)
{
    ...
    addSpring(g1b)
    eBt <- gbutton(gettext("Edit", domain = "R-DataEntry"), container = g1b)
    dBt <- gbutton(gettext("Delete", domain = "R-DataEntry"), container = g1b)
    aBt <- gbutton(gettext("Add", domain = "R-DataEntry"), container = g1b)

    addSpring(g2)
    upBt <- gbutton("^", container = g2)
    addSpace(g2, 10)
    dwBt <- gbutton("v", container = g2)
    addSpring(g2)
    ...
    addSpring(g2b)
    clsBt <- gbutton(gettext("Close", domain = "R-DataEntry"), container = g2b)

    addHandlerClicked(eBt, function(...) AttrDlg(FALSE))
    addHandlerClicked(aBt, function(...) AttrDlg(TRUE))
    addHandlerClicked(dBt, DeleteVariable)
    addHandlerClicked(DEenv$vlist, ShowAttributes)
    addHandlerClicked(upBt, function(...) MoveUpDown(TRUE))
    addHandlerClicked(dwBt, function(...) MoveUpDown(FALSE))
    addHandlerClicked(clsBt, function(...) dispose(DEenv$varw))
